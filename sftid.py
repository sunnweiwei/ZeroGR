...
class TrainGenData(Dataset):
    ...
    def __getitem__(self, index):
        question = self.data[index]["input"]
        answer = self.data[index]["output"]

        question = self.tokenizer.decode(
            self.tokenizer.encode(question)[:256], skip_special_tokens=True
        )
        inst = (
            self.data[index]["instruction"] if "instruction" in self.data[index] else ""
        )
        inst = self.tokenizer.decode(
            self.tokenizer.encode(inst)[:200], skip_special_tokens=True
        )
        question = inst + "\n\nDocument:\n" + question
...
def main():
    ...
    save_path = "models/Llama-3.2-1B-Instruct-ID"
